$(function () {
	...
	UE.delEditor('notice_noticeContent');
	var notice_noticeContent_editor = UE.getEditor('notice_noticeContent'); //公告内容编辑框
	$("#notice_noticeTitle").validatebox({
		required : true, 
		missingMessage : '请输入标题',
	});

	$("#notice_noticeClass").validatebox({
		required : true, 
		missingMessage : '请输入公告类别',
	});

	$("#notice_addDate").datebox({
	    required : true, 
	    showSeconds: true,
	    editable: false
	});
