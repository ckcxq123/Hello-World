$(function () {
	...
	UE.delEditor('notice_noticeContent_edit');
	var notice_noticeContent_edit = UE.getEditor('notice_noticeContent_edit'); //公告内容编辑器
	notice_noticeContent_edit.addListener("ready", function () {
		 // editor准备好之后才可以使用 
		 ajaxModifyQuery();
	}); 
	...
	$.ajax({
		...
		success : function (notice, response, status) {
			...
			if (notice) { 
				...
				$("#notice_noticeTitle_edit").validatebox({
					required : true,
					missingMessage : "请输入标题",
				});
				$("#notice_noticeClass_edit").val(notice.noticeClass);
				$("#notice_noticeClass_edit").validatebox({
					required : true,
					missingMessage : "请输入公告类别",
				});
				notice_noticeContent_edit.setContent(notice.noticeContent);
				$("#notice_addDate_edit").datebox({
					value: notice.addDate,
					required: true,
					showSeconds: true,
				});
			} else {
