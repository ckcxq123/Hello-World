...
public class AdoptController extends BaseController {
	...
	@InitBinder("petObj")//初始化
	public void initBinderpetObj(WebDataBinder binder) {//webDataBinder是用于表单到方法的数据绑定
	...
	public void userAdd(Adopt adopt,
		...
		if(adoptService.queryAdopt(adopt.getPetObj(), userObj, "", "").size() > 0) {
			message = "你已经申请过这个宠物领养了！";
			writeJsonResponse(response, success, message);
			return ;	
		}
		
		SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
		adopt.setAddTime(sdf.format(new java.util.Date()));
		adopt.setPetObj(adopt.getPetObj());
		adopt.setUserObj(userObj);
		adopt.setShenHe("待审核");
        adoptService.addAdopt(adopt);
        message = "领养添加成功!";
        success = true;
	...
	/*用户前台按照查询条件分页查询自己的领养信息*/
	@RequestMapping(value = { "/frontUserlist" }, method = {RequestMethod.GET,RequestMethod.POST})
	public String frontUserlist(@ModelAttribute("petObj") Pet petObj,@ModelAttribute("userObj") UserInfo userObj,String addTime,String shenHe,Integer currentPage, Model model, HttpServletRequest request,HttpSession session) throws Exception  {
		if (currentPage==null || currentPage == 0) currentPage = 1;
		if (addTime == null) addTime = "";
		if (shenHe == null) shenHe = "";
	    ...
	    List<UserInfo> userInfoList = userInfoService.queryAllUserInfo();
	    request.setAttribute("userInfoList", userInfoList);
		return "Adopt/adopt_frontUserquery_result"; 
	}
