...
public class FoodOrderController extends BaseController {
	...
	/*前台用户客户端ajax方式提交添加宠粮订单信息*/
	@RequestMapping(value = "/userAdd", method = RequestMethod.POST)
	public void userAdd(FoodOrder foodOrder,
		...
		int foodId = foodOrder.getFoodObj().getFoodId();
		Food food = foodService.getFood(foodId);
		if(foodOrder.getOrderNumber() > food.getFoodNum()) {
			message = "库存不能满足你的需求量！";
			writeJsonResponse(response, success, message);
			return ;
		}
		 
		foodOrder.setUserObj(userObj);
		SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
		foodOrder.setOrderTime(sdf.format(new java.util.Date()));
		foodOrder.setOrderState("待发货");
		 
        foodOrderService.addFoodOrder(foodOrder);
        food.setFoodNum(food.getFoodNum() - foodOrder.getOrderNumber()); //更新库存
	...
	/*用户前台按照查询条件分页查询自己的宠粮订单信息*/
	@RequestMapping(value = { "/frontUserlist" }, method = {RequestMethod.GET,RequestMethod.POST})
	public String frontUserlist(@ModelAttribute("foodObj") Food foodObj,@ModelAttribute("userObj") UserInfo userObj,String orderState,String orderTime,Integer currentPage, Model model, HttpServletRequest request,HttpSession session) throws Exception  {
		if (currentPage==null || currentPage == 0) currentPage = 1;
		if (orderState == null) orderState = "";
		if (orderTime == null) orderTime = "";
	    ...
	    List<UserInfo> userInfoList = userInfoService.queryAllUserInfo();
	    request.setAttribute("userInfoList", userInfoList);
		return "FoodOrder/foodOrder_frontUserquery_result"; 
	}
